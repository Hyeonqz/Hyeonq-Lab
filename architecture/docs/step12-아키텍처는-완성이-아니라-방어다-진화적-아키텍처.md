...
- **계약 테스트.** 서비스 간 API 계약(소비자 주도 계약, Pact류)을 자동 검증 — 발행자가 계약을 깨면 소비자의 빌드가 아니라 **발행자의 빌드가** 깨진다.
...
첫 번째 규칙이 깨졌다는 것은 "도메인이 세부사항의 인질이 되기 시작했다"(Step 06의 병)는 조기 경보이고, 두 번째는 "경계가 소멸하고 있다"(Step 03의 ADP)는 경보다. 마감에 몰린 "이번만"은 이제 리뷰어의 주의력이 아니라 **빨간 빌드**와 협상해야 한다. 모듈러 모놀리스(Step 10)에서 "경계 위반이 빌드에서 깨진다"고 했던 그 규율의 실체가 이것이다 — Spring Modulith의 모듈 검증(`ApplicationModules.verify()`)도 같은 계열의 도구다.
