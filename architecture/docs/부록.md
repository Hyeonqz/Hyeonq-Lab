...
문서화의 우선순위는 이 책의 논지대로다: 낡지 않는 것부터. **적합도 함수(낡으면 깨진다) > ADR(불변이므로 낡지 않는다) > 다이어그램(그린 날부터 낡는다).** 다이어그램은 필요하되, 진실의 원천이 아니라 안내판으로 취급하라.
...
| 연도 | 문헌/사건 | 핵심 | 장 |
|---|---|---|---|
| 1962~64 | Amdahl, Blaauw & Brooks, IBM System/360 | "아키텍처"의 컴퓨팅 정착 — 구현과 분리된 약속 | 01 |
